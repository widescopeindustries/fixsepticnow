...
export default async function ServiceDetailPage({ params }: { params: Promise<{ city: string; service: string }> }) {
  ...
  const comboFaqs = comboContent?.faqs || [
    ...
    { question: `Who provides ${service.shortName.toLowerCase()} in ${city.name}?`, answer: `Fix Septic Now provides licensed, insured ${service.shortName.toLowerCase()} in ${city.name} and all of ${city.county} County. Our technicians are experienced with the ${city.soilType} soil conditions in your area. Call (469) 986-7883.` },
    ...
    { question: `Is ${service.shortName.toLowerCase()} available on weekends in ${city.name}?`, answer: `Yes. We offer ${service.shortName.toLowerCase()} in ${city.name} seven days a week, including weekends and holidays. Emergency service is available 24/7. Call (469) 986-7883 to schedule.` },
  ];
    ...
    <>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-4xl md:text-5xl font-black leading-tight mb-4">
                {service.isEmergency ? `${service.name} in ${city.name}, TX — Call Now` : `${service.name} in ${city.name}, TX — 24/7 Same-Day`}
              </h1>
      ...
      <section className="py-10 bg-white">
        <div className="max-w-3xl mx-auto px-4 space-y-6">
          <QuickAnswer
            ...
            answer={`${service.name} in ${city.name} typically costs ${service.priceRange}. Pricing depends on tank size, accessibility, and the ${city.soilType} soil conditions in ${city.county} County. Call (469) 986-7883 for a free estimate — emergency service available 24/7.`}
          />
      ...
      <section className="py-12 bg-white">
        <div className="max-w-5xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-4">Service Area in {city.name}</h2>
